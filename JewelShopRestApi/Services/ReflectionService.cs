...
namespace JewelShopRestApi.Services
{
    public class ReflectionService
    {
        public List<string> GetInfoByAssembly()
        {
            ...
            Assembly assembly = GetAssembly("JewelShopService");
            List<Type> interfaces = GetInterfases(assembly);
            foreach (var inter in interfaces)
            {
                CustomAttributeData attr = inter.CustomAttributes.FirstOrDefault(x => x.AttributeType.Name == "BuyerInterfaceAttribute");
                if (attr != null)
                {
        ...
        private List<string> GetMethodInfoes(Type inter)
        {
            ...
            {
                CustomAttributeData attr = methods[i].CustomAttributes.FirstOrDefault(x => x.AttributeType.Name == "BuyerMethodAttribute");
                if (attr != null)
                {
